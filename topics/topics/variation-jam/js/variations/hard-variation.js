...
let weight = 20; //size of snake and food
let total = 0;
let news = new Snake();
let ate;
...
function pickLocation() {
    var cols = floor(width / weight);
    var rows = floor(height / weight);
    ate = createVector(floor(random(cols)), floor(random(rows)));//this ensure the food is in the grid aligned with snake
    ate.mult(weight);//to expand it back out
}
...
function hardDraw() {
    ...
    rect(ate.x, ate.y, weight, weight);
    pop();
...
function hardKeyPressed(event) {
    ...
    if (keyCode === UP_ARROW) {
        news.dir(0, -1); //moves 0 along x and -1 (up) along y axis
...
function Snake() {
    ...
    this.update = function () {
        ...
        this.x = this.x + this.xspeed * weight;
        this.y = this.y + this.yspeed * weight;
    ...
    this.show = function () {
        ...
        for (var i = 0; i < this.tail.length; i++) {
            rect(this.tail[i].x, this.tail[i].y, weight, weight);
        }
